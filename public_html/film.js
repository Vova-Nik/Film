...
*/
    ...
    //Cheking if there enought room to place picture of required size
    check_place(elem_wids) {
        try {
            if (this.pole_arr[this.pty][this.ptx] === 1) {
                return (false);
            ...
            if (this.pole_arr[this.pty + elem_wids][this.ptx] === 1) {
                return (false);
            }
            if (this.pole_arr[this.pty + elem_wids][this.ptx + elem_wids] === 1) {
                return (false);
    ...
    //Find place on screen for picture with width = heigth = element_width
    place(element_width) {
        ...
        do {
            ...
            if (this.pt_inc() === false) {
                ...
                return (this.point);
                alert('Dead branch in pole->place');
            }
        } while (true);
    }
...
class Film {
    constructor(ii, arr)
    {
        try {
            this.id = ii;
            this.name = arr[ii].name;
            this.name_pl = arr[ii].name;
            this.name_ua = arr[ii].name;
            this.name_ru = arr[ii].name;
            this.idb = arr[ii].rate;
            this.img_url = arr[ii].image;
            console.log('Film construcnor ' + this.name + ' ' + this.idb + ' ' + this.img_url);
        }
        catch (err) {
        ...
        if (!(+this.idb)) {
            this.idb = 1;
            //alert("Film constructor error idb not correct");
            console.log("Film constructor error idb not correct");
        }
        ...
        this.year = 2010;
        this.actualTypes = 'Good movie';
        this.ptx = 0; // x coord for place seeking
        this.pty = 0; // y coord for place seeking
        this.squod = "squod" + this.id; 
    }
    /* old constructor for work wis server
    ...
    */
    ...
    show(x, y) {
        console.log('show(x,y) processing');
        //var squod = "squod" + this.id;
        //let imurl = 'url(/img/' + this.img_url + ')'; 
        let imurl = `url(/img/${this.img_url})`;
        ...
        // document.getElementById(this.squod).innerHTML = (`<p>X =   ${this.x_coord8} </p> <p> Y = ${this.y_coord8}</p><p>${imurl}</p>`);
...
//instead of fuOnRequestRedy() when working out of server
function init(){ 
    console.log("Init processing");
    numberOfFilms = movies.length;
    console.log("Number of films  " + numberOfFilms + "\n");
    for (var i = 0; i < numberOfFilms; i++) {
        let film = new Film(i, movies);
        films.push(film);
        //films[i].ShowInner();
    }
    films.shuffle();
...
// let movies = [
//     {name: 'Terminator', rate: 7, image: '000.jpg'},
//     {name: 'bbb', rate: 7, image: '001.jpg'},
//     {name: 'ccc', rate: 7, image: '002.jpg'},
//     {name: 'ddd', rate: 8, image: '003.jpg'},
//     {name: 'eee', rate: 8, image: '004.jpg'},
//     {name: 'fff', rate: 9, image: '005.jpg'},
//     {name: 'ggg', rate: 9, image: '006.jpg'},
//     {name: 'hhh', rate: 9, image: '007.jpg'},
//     {name: 'iii', rate: 9.5, image: '008.jpg'},
// ];
